...
void Scene::createWindow()
{
	...
	glLoadIdentity(); // *** Added this bit of code because the original matrix does not seem to be defaulted to the indentity (at least on my machine).
...
void Scene::draw()
{
	for (vector<Graphic*>::iterator iterator = graphics.begin() ; iterator != graphics.end(); iterator++)
		(*iterator)->draw();
}